...
> \*\*Note:\*\* `AccountNumber` serves as the \*\*Primary Key (PK)\*\* in the `ACCOUNT` table and acts as a \*\*Foreign Key (FK)\*\* in both `SAVINGS\_ACCOUNT` and `CHECKING\_ACCOUNT` tables.
...
\- Savings-specific information is stored in the `SAVINGS\_ACCOUNT` table.

\- Checking-specific information is stored in the `CHECKING\_ACCOUNT` table.
